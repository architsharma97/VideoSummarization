...
%% Generate random segmentation results
%
shot_length = 50;
segment_results = cell(1,n_videos);
for i=1:n_videos,
    shot_boundaries = shot_length:shot_length:tvsum50(i).nframes;
    shot_boundaries = shot_boundaries + randi(30,size(shot_boundaries))-15;
...
for i = 1:size(results,1) 
    ...
    for j = 1:size(results,2) 
        ...
        stats{i,j}.prec = cellfun(@(x) x.PositivePredictiveValue, cp);
        stats{i,j}.rec  = cellfun(@(x) x.Sensitivity, cp);
        stats{i,j}.f1 = max(0,2*(stats{i,j}.prec.*stats{i,j}.rec) ./...
            (stats{i,j}.prec+stats{i,j}.rec));
...
for i = 1:numel(categories),
    ...
    for j = 1:numel(videos),
        per_category_perf{i}(:,j) = ...
            stats_mean_f1(:,find(strcmp(stats_video,videos{j})));
    end
end
